...
$query=mysqli_query($con,"select * from planes AS p INNER JOIN plan_cliente AS z
  ...
  $cantidad_enero_productos=0;
    $cantidad_febrero_productos=0;
      ...
      ON p.id_pedido = z.id_pedido INNER JOIN producto AS c
      ON c.id_pro = z.id_producto   where p.fecha >='$enero_inicio' and p.fecha <='$enero_fin' ")or die(mysqli_error());
      while($row=mysqli_fetch_array($query)){
...
$query=mysqli_query($con,"select * from pedidos AS p INNER JOIN detalles_pedido AS z
      ON p.id_pedido = z.id_pedido INNER JOIN producto AS c
      ON c.id_pro = z.id_producto    where p.fecha >='$febrero_inicio' and p.fecha <='$febrero_fin'")or die(mysqli_error());
      while($row=mysqli_fetch_array($query)){
     $cantidad_febrero=$row['precio_venta']*$row['cantidad']+$cantidad_febrero;
      ...
      $query=mysqli_query($con,"select * from pedidos AS p INNER JOIN detalles_pedido AS z
      ON p.id_pedido = z.id_pedido INNER JOIN producto AS c
      ON c.id_pro = z.id_producto    where p.fecha >='$marzo_inicio' and p.fecha <='$marzo_fin'")or die(mysqli_error());
      while($row=mysqli_fetch_array($query)){
            ...
            $query=mysqli_query($con,"select * from pedidos AS p INNER JOIN detalles_pedido AS z
      ON p.id_pedido = z.id_pedido INNER JOIN producto AS c
      ON c.id_pro = z.id_producto   where p.fecha >='$abril_inicio' and p.fecha <='$abril_fin'")or die(mysqli_error());
      while($row=mysqli_fetch_array($query)){
           ...
           $query=mysqli_query($con,"select * from pedidos AS p INNER JOIN detalles_pedido AS z
      ON p.id_pedido = z.id_pedido INNER JOIN producto AS c
      ON c.id_pro = z.id_producto   where p.fecha >='$mayo_inicio' and p.fecha <='$mayo_fin'")or die(mysqli_error());
      while($row=mysqli_fetch_array($query)){
          ...
          $query=mysqli_query($con,"select * from pedidos AS p INNER JOIN detalles_pedido AS z
      ON p.id_pedido = z.id_pedido INNER JOIN producto AS c
      ON c.id_pro = z.id_producto   where p.fecha >='$junio_inicio' and p.fecha <='$junio_fin'")or die(mysqli_error());
      while($row=mysqli_fetch_array($query)){
        ...
        $cantidad_junio=$row['precio_venta']*$row['cantidad']+$cantidad_junio;
         ...
         $query=mysqli_query($con,"select * from pedidos AS p INNER JOIN detalles_pedido AS z
      ON p.id_pedido = z.id_pedido INNER JOIN producto AS c
      ON c.id_pro = z.id_producto    where p.fecha >='$julio_inicio' and p.fecha <='$juio_fin'")or die(mysqli_error());
      while($row=mysqli_fetch_array($query)){
        ...
        $query=mysqli_query($con,"select * from pedidos AS p INNER JOIN detalles_pedido AS z
      ON p.id_pedido = z.id_pedido INNER JOIN producto AS c
      ON c.id_pro = z.id_producto   where p.fecha >='$agosto_inicio' and p.fecha <='$agosto_fin'")or die(mysqli_error());
      while($row=mysqli_fetch_array($query)){
        $cantidad_agosto=$row['precio_venta']*$row['cantidad']+$cantidad_agosto;
              ...
              $query=mysqli_query($con,"select * from pedidos AS p INNER JOIN detalles_pedido AS z
      ON p.id_pedido = z.id_pedido INNER JOIN producto AS c
      ON c.id_pro = z.id_producto   where p.fecha >='$setiembre_inicio' and p.fecha <='$setiembre_fin'")or die(mysqli_error());
      while($row=mysqli_fetch_array($query)){
           $cantidad_setiembre=$row['precio_venta']*$row['cantidad']+$cantidad_setiembre;
             ...
             $query=mysqli_query($con,"select * from pedidos AS p INNER JOIN detalles_pedido AS z
      ON p.id_pedido = z.id_pedido INNER JOIN producto AS c
      ON c.id_pro = z.id_producto    where p.fecha >='$octubre_inicio' and p.fecha <='$octubre_fin'")or die(mysqli_error());
      while($row=mysqli_fetch_array($query)){
            ...
            $query=mysqli_query($con,"select * from pedidos AS p INNER JOIN detalles_pedido AS z
      ON p.id_pedido = z.id_pedido INNER JOIN producto AS c
      ON c.id_pro = z.id_producto   where p.fecha >='$noviembre_inicio' and p.fecha <='$noviembre_fin'")or die(mysqli_error());
      while($row=mysqli_fetch_array($query)){
         $cantidad_noviembre=$row['precio_venta']*$row['cantidad']+$cantidad_noviembre;
           ...
           $query=mysqli_query($con,"select * from pedidos AS p INNER JOIN detalles_pedido AS z
      ON p.id_pedido = z.id_pedido INNER JOIN producto AS c
      ON c.id_pro = z.id_producto    where p.fecha >='$diciembre_inicio' and p.fecha <='$diciembre_fin'")or die(mysqli_error());
      while($row=mysqli_fetch_array($query)){
...
$query=mysqli_query($con,"select * from gastos  where fecha >='$febrero_inicio' and fecha <='$febrero_fin'")or die(mysqli_error());
   ...
   $cantidad_febrero_gastos=$row['cantidad']+$cantidad_febrero_gastos;
      ...
      $query=mysqli_query($con,"select * from gastos where fecha >='$marzo_inicio' and fecha <='$marzo_fin'")or die(mysqli_error());
      while($row=mysqli_fetch_array($query)){
   $cantidad_marzo_gastos=$row['cantidad']+$cantidad_marzo_gastos;

      }

            $query=mysqli_query($con,"select * from gastos where fecha >='$abril_inicio' and fecha <='$abril_fin'")or die(mysqli_error());
      while($row=mysqli_fetch_array($query)){
   $cantidad_abril_gastos=$row['cantidad']+$cantidad_abril_gastos;
      }
     
           $query=mysqli_query($con,"select * from gastos where fecha >='$mayo_inicio' and fecha <='$mayo_fin'")or die(mysqli_error());
      while($row=mysqli_fetch_array($query)){
   $cantidad_mayo_gastos=$row['cantidad']+$cantidad_mayo_gastos;

      }

          $query=mysqli_query($con,"select * from gastos  where fecha >='$junio_inicio' and fecha <='$junio_fin'")or die(mysqli_error());
      while($row=mysqli_fetch_array($query)){

   $cantidad_junio_gastos=$row['cantidad']+$cantidad_junio_gastos;
      }

         $query=mysqli_query($con,"select * from gastos  where fecha >='$julio_inicio' and fecha <='$juio_fin'")or die(mysqli_error());
      while($row=mysqli_fetch_array($query)){
   $cantidad_julio_gastos=$row['cantidad']+$cantidad_julio_gastos;

      }

        $query=mysqli_query($con,"select * from gastos  where fecha >='$agosto_inicio' and fecha <='$agosto_fin'")or die(mysqli_error());
      while($row=mysqli_fetch_array($query)){
   $cantidad_agosto_gastos=$row['cantidad']+$cantidad_agosto_gastos;
 
      }
              $query=mysqli_query($con,"select * from gastos where fecha >='$setiembre_inicio' and fecha <='$setiembre_fin'")or die(mysqli_error());
      while($row=mysqli_fetch_array($query)){
   $cantidad_setiembre_gastos=$row['cantidad']+$cantidad_setiembre_gastos;

      }

             $query=mysqli_query($con,"select * from gastos  where fecha >='$octubre_inicio' and fecha <='$octubre_fin'")or die(mysqli_error());
      while($row=mysqli_fetch_array($query)){
   $cantidad_octubre_gastos=$row['cantidad']+$cantidad_octubre_gastos;
 
      }

            $query=mysqli_query($con,"select * from gastos where fecha >='$noviembre_inicio' and fecha <='$noviembre_fin'")or die(mysqli_error());
      while($row=mysqli_fetch_array($query)){
   $cantidad_noviembre_gastos=$row['cantidad']+$cantidad_noviembre_gastos;

      }

           $query=mysqli_query($con,"select * from gastos  where fecha >='$diciembre_inicio' and fecha <='$diciembre_fin'")or die(mysqli_error());
      while($row=mysqli_fetch_array($query)){
